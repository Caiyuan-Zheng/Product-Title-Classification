...
def read_file(content_dir,max_data):
    """读取文件数据"""
    contents= []
    rank=0
    max_len=0
    readlines=open(content_dir,'r').readlines()
...
def process_file(content_dir, word_to_id, cat_to_id, max_length=25,max_data=10000):
    """将文件转换为id表示"""
    contents = read_file(content_dir,100000000)
    chars=list('abcdefghijklmnopqrstuvwxyz我的于超前越你及等免二非下单五元')
    chars+=['新款','mm','送','新','可','一','不','与','和','无','有','之','自','好','适用',]
    data_id =  []
...
def predict(classifier,sess,x_test,id_to_label,batch_size=1024):
    start=0
    end=0
    y_test=[]
    rank=0
    while end<len(x_test):
        ...
        y_pred_cls = sess.run(classifier.prediction, feed_dict=feed_dict)
        y_test.extend(y_pred_cls)
        rank+=1
        if rank%200==0:
            print (rank)
    assert len(x_test)==len(y_test)
    y_text=[id_to_label[id] for id in y_test]
    fout=open('data/train-error1-predict.txt','w')
    fout.write('\n'.join(y_text))
    fout.close()
...
if __name__ == '__main__':
    ...
    if os.path.exists('data/embedding-char.pkl'):
        with open('data/embedding-char.pkl','rb') as f:
            flags.word_embedding=pickle.load(f)
    else:
        flags.word_embedding=create_embedding(words)
        with open('data/embedding-char.pkl','wb') as f:
            pickle.dump(flags.word_embedding,f)

    x_test,y_test=None,None
    x_train,y_train=None,None
    x_tmp,y_tmp=None,None
    flags.word_max_len=45
